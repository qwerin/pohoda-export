...
class Order
{
	...
	{
		$xmlInvoice = $xml->addChild("ord:order", null, self::NS);
		...
		if($this->cancel) {
			...
			$this->exportCancelHeader($xmlInvoice->addChild("ord:orderHeader", null, self::NS));
		} else {
			$this->exportHeader($xmlInvoice->addChild("ord:orderHeader", null, self::NS));
			if (!empty($this->items)) {
				$this->exportDetail($xmlInvoice->addChild("ord:orderDetail", null, self::NS));
			}
			$this->exportSummary($xmlInvoice->addChild("ord:orderSummary", null, self::NS));
		}
	...
	private function exportHeader(SimpleXMLElement $header)
	{
		...
		$num = $header->addChild("ord:number");
		...
		if (!is_null($this->dateTax))
			$header->addChild("ord:dateTax", $this->dateTax);
		if (!is_null($this->dateDue))
			$header->addChild("ord:dateDue", $this->dateDue);
		if (!is_null($this->dateAccounting))
			$header->addChild("ord:dateAccounting", $this->dateAccounting);
